...
export default function DashboardPage() {
  ...
  const [metrics, setMetrics] = useState<DashboardMetrics>({ sales: null, activeOrders: null, kitchenOrders: null, cashOpen: null, lowStock: null, averageTicket: null, productCount: null, categoryCount: null });
  const [metricStates, setMetricStates] = useState<Record<DashboardMetricKey, MetricState>>({ sales: "loading", activeOrders: "loading", kitchenOrders: "loading", cashOpen: "loading", lowStock: "loading", averageTicket: "loading", productCount: "loading", categoryCount: "loading" });
    ...
    async function loadDashboard() {
      try {
        ...
        const [products, categories, orders, kitchenOrders, cash, report, inventory] = await Promise.all([
          canCatalog ? fetchMetric<ProductResponse>("/api/v1/products?limit=1") : Promise.resolve(forbiddenMetric<ProductResponse>()),
          canCatalog ? fetchMetric<CategoryResponse>("/api/v1/categories") : Promise.resolve(forbiddenMetric<CategoryResponse>()),
          canOrders ? fetchMetric<{ data: Order[] }>("/api/v1/orders") : Promise.resolve(forbiddenMetric<{ data: Order[] }>()),
          canKitchen ? fetchMetric<{ data: KitchenOrder[] }>("/api/v1/kitchen/orders") : Promise.resolve(forbiddenMetric<{ data: KitchenOrder[] }>()),
          canCash ? fetchMetric<CashResponse>("/api/v1/cash/summary") : Promise.resolve(forbiddenMetric<CashResponse>()),
          canCash ? fetchMetric<ReportResponse>(`/api/v1/reports/summary?date=${businessDate()}`) : Promise.resolve(forbiddenMetric<ReportResponse>()),
          canKitchen ? fetchMetric<InventoryResponse>("/api/v1/inventory") : Promise.resolve(forbiddenMetric<InventoryResponse>()),
        ]);
        ...
        setMetrics({
          ...
          lowStock: inventory.data?.data.filter((item) => Number(item.current_stock) <= Number(item.minimum_stock)).length ?? null,
  ...
  const cards = [
    { label: "Ventas de hoy", value: displayMetric(metrics.sales, metricStates.sales, (value) => `S/ ${value.toFixed(2)}`), detail: "Pedidos pagados" },
    { label: "Pedidos activos", value: displayMetric(metrics.activeOrders, metricStates.activeOrders), detail: "En operación" },
    { label: "En cocina", value: displayMetric(metrics.kitchenOrders, metricStates.kitchenOrders), detail: "Comandas activas" },
    { label: "Caja", value: displayMetric(metrics.cashOpen, metricStates.cashOpen, (value) => value ? "Abierta" : "Cerrada"), detail: "Estado del turno" },
    { label: "Bajo stock", value: displayMetric(metrics.lowStock, metricStates.lowStock), detail: "Insumos por revisar" },
    { label: "Ticket promedio", value: displayMetric(metrics.averageTicket, metricStates.averageTicket, (value) => `S/ ${value.toFixed(2)}`), detail: "Pedidos pagados" },
  ];
  ...
  return (
    <main className="shell dashboard-shell">
      ...
      <section className="dashboard-welcome"><div><div className="eyebrow">Panel operativo</div><h1>Hola, {emailName}</h1><p>{isKitchenUser ? "Entra a tu pantalla de trabajo." : "Este es el pulso de tu negocio hoy."}</p></div><span className="dashboard-role">{session?.roles.join(" · ") ?? ""}</span></section>
      {metricsWarning && <p className="dashboard-warning" role="status">Algunas métricas no pudieron actualizarse. Revisa la conexión o los permisos.</p>}
      <section className="dashboard-actions" aria-label="Acciones principales">{canCreateOrder && <Link className="button button-primary" href="/dashboard/orders">+ Nuevo pedido</Link>}{canOpenCash && <Link className="button button-secondary" href="/dashboard/cash">Abrir caja</Link>}</section>
      <section className="dashboard-metrics" aria-label="Resumen operativo">{cards.map((card) => <article className="dashboard-metric" key={card.label}><span>{card.label}</span><strong>{card.value}</strong><small>{card.detail}</small></article>)}</section>
      <section className="dashboard-modules" aria-label="Módulos disponibles">{visibleModules.map((module) => <Link className="dashboard-module active" href={module.href} key={module.label}><span className="module-icon">{module.icon}</span><span><strong>{module.label}</strong><small>{module.description}</small></span><b>›</b></Link>)}</section>
      <section className="dashboard-summary" aria-label="Recursos del sistema"><div><span>Productos</span><strong>{displayMetric(metrics.productCount, metricStates.productCount)}</strong></div><div><span>Categorías</span><strong>{displayMetric(metrics.categoryCount, metricStates.categoryCount)}</strong></div><div><span>Roles activos</span><strong>{session?.roles.length ?? "—"}</strong></div></section>
    </main>
  );
}

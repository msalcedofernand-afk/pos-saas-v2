...
export default function ProductsPage() {
  ...
  function openEdit(product: Product) {
    ...
    setForm({
      ...
      prepTimeMinutes: String(product.prep_time_minutes ?? 0),
    });
    setFormError(null);
    setModalOpen(true);
  }
  ...
  async function saveProduct(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await apiFetch(editing ? `/api/v1/products/${editing.id}` : "/api/v1/products", {
  ...
  async function removeProduct(product: Product) {
    ...
    try {
      ...
      window.alert(cause instanceof Error ? cause.message : "No se pudo eliminar el producto");
    }
  }
  ...
  async function createCategory() {
    ...
    if (!name) return;
    try {
      const response = await apiFetch<{ data: Category }>("/api/v1/categories", {
        method: "POST",
      ...
      window.alert(cause instanceof Error ? cause.message : "No se pudo crear la categoría");
    }
  }
  ...
  return (
    <main className="shell catalog-shell">
      <nav className="nav catalog-nav">
        ...
        <div className="nav-actions"><strong>Productos</strong><Link className="button button-small" href="/dashboard">Volver</Link></div>
      </nav>

      <div className="catalog-heading">
        <div><div className="eyebrow">Carta y disponibilidad</div><h1>Productos</h1><p className="lead">Administra lo que aparece en la carta y controla qué puede venderse hoy.</p></div>
        {canManage && <button className="button button-primary" onClick={openCreate}>+ Nuevo producto</button>}
      </div>

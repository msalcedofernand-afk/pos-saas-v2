  ...
  return (
    <main className="shell catalog-shell">
      ...
      <div className="catalog-heading">
        <div>
          <div className="eyebrow">Carta y disponibilidad</div>
          <h1>Productos</h1>
          <p className="lead">Administra lo que aparece en la carta y controla qué puede venderse hoy.</p>
        </div>
        {canManage && (
      ...
      <section className="catalog-toolbar">
        ...
        <div className="category-filter">
          ...
          {canManage && (
            <button className="link-button" onClick={() => setCategoryModalOpen(true)} type="button">
              + Categoría
            </button>
          )}
        </div>
        ...
        <select id="availability-filter" value={availability} onChange={(event) => setAvailability(event.target.value)}>
      ...
      <section className="product-table-section">
        {loading ? (
          ...
          <p className="empty-state">No hay productos con estos filtros.</p>
        ) : (
              ...
              <tbody>
                {products.map((product) => (
                  <tr key={product.id}>
                    ...
                    <td>{product.prep_time_minutes || 0} min</td>
                    <td>
                      <span className={product.is_available ? "status-pill available" : "status-pill unavailable"}>
                    ...
                    <td>
                      <div className="row-actions">
                        {canManage && (
                          <button className="link-button" onClick={() => openEdit(product)} type="button">
                            Editar
                          </button>
                        )}
                        {canDelete && (
                          <button className="link-button danger" onClick={() => setDeleteTarget(product)} type="button">
                            Eliminar
                          </button>
                        )}
                      </div>
      ...
      {modalOpen && (
        <div className="modal-backdrop">
          <form aria-labelledby="product-dialog-title" className="product-modal" onSubmit={saveProduct} role="dialog">
            <div className="modal-heading">
              <div>
                <div className="eyebrow">{editing ? "Editar producto" : "Nuevo producto"}</div>
                <h2 id="product-dialog-title">{editing ? editing.name : "Agregar a la carta"}</h2>
              </div>
              <button
      ...
      {categoryModalOpen && (
        <div className="modal-backdrop">
          ...
          >
            ...
            <div className="modal-actions">
              <button type="button" className="button button-secondary" onClick={() => setCategoryModalOpen(false)}>
                Cancelar
              </button>
              <button className="button button-primary">Guardar categoría</button>
            </div>
          </form>
        </div>
      )}
      <ConfirmDialog
        description={deleteTarget ? `Se eliminará ${deleteTarget.name} de la carta.` : ""}
        busy={deleteBusy}

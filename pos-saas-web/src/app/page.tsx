...
const benefits = [
  { title: "Toma pedidos sin duplicar trabajo", text: "Meseros, caja y cocina comparten el mismo pedido desde el primer clic." },
  { title: "Cocina siempre sabe qué preparar", text: "Las comandas llegan ordenadas por estación y estado, sin papeles perdidos." },
    ...
    <main className="landing-shell">
      ...
      <section className="hero landing-hero">
        <div>
          ...
          <p className="lead">Pedidos conectados con cocina, caja e inventario para que tu equipo atienda mejor y tú tengas el control del turno.</p>
          <div className="actions"><Link className="button button-primary" href="/demo">Ver demo</Link><Link className="button button-secondary" href="/login">Ingresar al panel</Link></div>
        ...
        <div className="preview landing-preview" aria-label="Vista previa de la operación">
          <div className="preview-top"><span>Turno de hoy</span><span className="preview-live">● En vivo</span></div>
          <div className="metric"><div className="metric-label">Ventas del día</div><div className="metric-value">S/ 4,280</div><span className="preview-trend">↑ 18% vs. ayer</span></div>
          <div className="landing-order-card"><div><strong>Pedido #1842</strong><small>Mesa 12 · Hace 4 min</small></div><span>En cocina</span></div>
          <div className="preview-row"><div className="mini-card"><b>24</b><small>pedidos</small></div><div className="mini-card"><b>8</b><small>mesas activas</small></div><div className="mini-card"><b>3</b><small>por preparar</small></div></div>
        </div>
      </section>

      <section className="benefits-section">
        <div className="section-heading landing-heading"><div><div className="eyebrow">Menos fricción, más servicio</div><h2>Todo el turno en una sola vista.</h2></div><p>La operación se mueve al ritmo de tu equipo, no al de una hoja de cálculo.</p></div>
        <div className="benefit-grid">{benefits.map((benefit, index) => <article className="benefit-card" key={benefit.title}><span>0{index + 1}</span><h3>{benefit.title}</h3><p>{benefit.text}</p></article>)}</div>
      </section>

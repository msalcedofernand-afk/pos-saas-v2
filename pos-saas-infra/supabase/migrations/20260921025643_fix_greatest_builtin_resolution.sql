...
BEGIN
  ...
  v_change := GREATEST(0::numeric, v_received - p_amount)::numeric(10,2);
  ...
  WHERE public.orders.id = p_order_id
    AND public.orders.organization_id = v_org_id;
  RETURN QUERY SELECT v_payment_id, p_order_id, p_method, p_amount, v_change, v_status, v_paid, GREATEST(0::numeric, v_order.total_amount - v_paid)::numeric(10,2);
END;
$$;

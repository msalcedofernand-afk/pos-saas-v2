...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: orderPayments, error: orderPaymentError } = orderIds.length === 0 ? { data: [], error: null } : await db.from("payments").select("order_id, amount").in("order_id", orderIds);
    ...
    const { data: payments, error: paymentError } = shift ? await db.from("payments").select("id, order_id, method, amount, change_amount, created_at").eq("shift_id", shift.id).order("created_at", { ascending: false }).limit(100) : { data: [], error: null };
